...
def render_profile():
    ...
    if message:
        ...
        st.markdown("📢 **管理员留言：**")
        st.info(message)
    predictions = load_user_predictions(username)
    if predictions:
        st.markdown("**📈 预测历史：**")
        for timestamp, pred in predictions.items():
            st.markdown(f"**时间:** {timestamp}, **概率:** {pred['probability']}%")
            if st.button(f"删除记录 ({timestamp})", key=f"delete_{timestamp}"):
                delete_prediction(username)
                st.rerun()  # 刷新页面以更新UI
    else:
        st.markdown("**尚未有预测记录。**")
    st.markdown("---")
...
def main():
    ...
    if not st.session_state.get('logged_in', False):
        ...
        # ========== 关键修改在这里 ==========
        # 如果不是用户主动从侧边栏切换页面，则使用 session_state 中的 page
        current_page = st.session_state.get('page', '登录')

        # 添加一个 checkbox 或按钮之类的 UI 组件（可选）
        with st.sidebar:
            st.title("🧭 导航")
            selection = st.radio("前往", list(pages.keys()), index=0 if current_page == "登录" else 1)

        # 每次更新 radio 后同步到 session_state
        st.session_state['page'] = selection

        # 页面路由
        if current_page == "登录":
            render_login()
        elif current_page == "注册":
            render_register()
